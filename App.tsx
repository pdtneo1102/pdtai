...
const SuperSharp = lazy(() => import('./components/SuperSharp'));
const SunEffectCreator = lazy(() => import('./components/SunEffectCreator'));
...
const AppComponents: Record<string, any> = {
    ...
    'super-sharp': { Component: SuperSharp, settingsKey: 'superSharp', props: (s:any, t:any) => ({ mainTitle: t(s.mainTitleKey), subtitle: t(s.subtitleKey), uploaderCaption: t(s.uploaderCaptionKey), uploaderDescription: t(s.uploaderDescriptionKey) }) },
    'sun-effect-creator': { Component: SunEffectCreator, settingsKey: 'sunEffectCreator', props: (s:any, t:any) => ({ mainTitle: t(s.mainTitleKey), subtitle: t(s.subtitleKey), uploaderCaption: t(s.uploaderCaptionKey), uploaderDescription: t(s.uploaderDescriptionKey) }) },
...
function App() {
    ...
    // License State
    const [forceShowLicenseModal, setForceShowLicenseModal] = useState(false);
    ...
    useEffect(() => {
        const hasSeenInfoModal = localStorage.getItem('PDTAI_hasSeenInfoModal');
        if (!hasSeenInfoModal) {
            handleOpenInfo();
            localStorage.setItem('PDTAI_hasSeenInfoModal', 'true');
        }
    }, []); 
    ...
    }, [isSearchOpen, isGalleryOpen, isInfoOpen, isHistoryPanelOpen, isImageLayoutModalOpen, isBeforeAfterModalOpen, isAppCoverCreatorModalOpen, isStoryboardingModalVisible, isLayerComposerVisible, imageToEdit, isLicenseModalOpen]);
        ...
        <main className="text-neutral-200 min-h-screen w-full relative">
            ...
            <LicenseModal 
                ...
                onLicenseValid={() => {
                    ...
                    setForceShowLicenseModal(false);
                }}
                forceShow={forceShowLicenseModal}
                onClose={() => {
                    setLicenseModalOpen(false);
                    setForceShowLicenseModal(false);
                }}

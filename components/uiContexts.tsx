...
import { validateLicenseKey, getSavedLicenseKey } from '../lib/license';
...
interface AuthContextType {
    ...
    // FIX: Change void to () => void to match implementation
    logout: () => void;
}
...
export const AppControlProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // FIXED: Only 'rainbow' theme is allowed.
    ...
    const checkLicenseAccess = useCallback(() => {
    ...
    useEffect(() => {
        // FIXED: Only add the rainbow theme class.
    ...
    const importSettingsAndNavigate = useCallback((settings: any) => {
        if (!checkLicenseAccess()) return; // License check for import
    ...
    }, [viewHistory, historyIndex, imageGallery, restoreStateFromGallery, checkLicenseAccess]);

    const handleSelectApp = useCallback((appId: string) => {
        // If not home, check license
        if (appId !== 'home' && !checkLicenseAccess()) {
            return;
        }
    ...
    }, [settings, navigateTo, checkLicenseAccess]);
    ...
    const value: AppControlContextType = {
        ...
        isLicenseValid,
        isLicenseModalOpen,
        setLicenseModalOpen: setIsLicenseModalOpen,
        checkLicenseAccess,
        validateLicense,
